...
contract BrokexStorage is Ownable {
    ...
    function deletePosition(
      ...
      // --- purge du flag de close request (irrévocable mais auto-nettoyé à la fermeture) ---
      if (closeRequested[positionId]) {
          delete closeRequested[positionId];
      }
  
      // --- efface l'état local de la position ---
      delete positions[positionId];
      _removeId(_traderPositions, p.trader, positionId);
  
      // --- règlement via le vault ---
